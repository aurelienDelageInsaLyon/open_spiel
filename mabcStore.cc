...
void MABCState::DoApplyAction(Action action) {
  ...
  //chance node
    ...
    if (rollout_chance_==1){
      ...
      ActionsAndProbs vec = ChanceOutcomes();
      std::cout << "\n chance outcomes : " << ChanceOutcomes();
      //auto it = std::find_if(vec.begin(),vec.end(), [&action] (const std::pair<Action, double> a) {return (a.first==action);});
      //rollout_proba_=it->second;
      for (auto& pair : vec){
        if (pair.first==rollout_action_){
          rollout_proba_ = pair.second;
        }
      }
      std::cout << "modified rollout_proba for : " << rollout_proba_;
      std::cout << "\n rolling in";
...
void MABCState::evolveState(const std::vector<Action>& actions, Action actionNature){
  ActionsAndProbs vec = ChanceOutcomes();
  std::cout << "\n chance outcomes : " << ChanceOutcomes();
  //auto it = std::find_if(vec.begin(),vec.end(), [&action] (const std::pair<Action, double> a) {return (a.first==action);});
  //rollout_proba_=it->second;
  double probReach = 0.0;
  for (auto& pair : vec){
    if (pair.first==actionNature){
      probReach = pair.second;
    }
  }
  std::cout << "\n probaReach :  " << probReach;
  ...
  for (std::pair<hiddenState,double> hiddenStatePair : distribHidden_States){
    ...
    if (ToAction(actions[0]) == ActionType::kSend &&
      ...
      if (hiddenState_ == hiddenState::s00){
        ...
        newDistribHidden_States[hiddenState::s01] += 0.81*distribHidden_States[hiddenState_];
        newDistribHidden_States[hiddenState::s10] += 0.01*distribHidden_States[hiddenState_];
    ...
    if (ToAction(actions[0]) == ActionType::kWait &&
      ...
      if (hiddenState_ == hiddenState::s10){
        newDistribHidden_States[hiddenState::s00] += 0.0*distribHidden_States[hiddenState_];
        newDistribHidden_States[hiddenState::s01] += 0.9*distribHidden_States[hiddenState_];
        newDistribHidden_States[hiddenState::s10] += 0.0*distribHidden_States[hiddenState_];
  ...
  std::cout << "\n state evolve in : " << map_to_string(distribHidden_States) << " with proba reach : " << probReach;
...
ActionsAndProbs MABCState::ChanceOutcomes() const {
  ...
  if (rollout_chance_==0){
    ...
    //std::map<int,float> rollout_list_proba_;
    //std::map<int,float> rollout_list_proba_={{0,0.0}};
    //storeChanceOutcomes = {0,distribObservation[0]+distribObservation[1]},{1,distribObservation[2]+distribObservation[3]};
    return {{0,distribObservation[0]+distribObservation[1]},{1,distribObservation[2]+distribObservation[3]}};
  }
  else{
    std::cout << "\n rollout proba : " <<rollout_proba_;
    return {{2,(distribObservation[0]+distribObservation[2])/rollout_proba_},{3,(distribObservation[1]+distribObservation[3])/rollout_proba_}};
  }*/
