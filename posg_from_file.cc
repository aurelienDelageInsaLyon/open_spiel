...
double posgFromFileState::resolveRewards(const std::vector<Action>& actions){
  ...
  for (int s=0;s<nbStates;s++){
    ...
    if (distribHidden_States[s]>0.0 && distribHidden_States[s] < kRMax*100){
      ...
      res +=distribHidden_States[s]*rewardFunction.at(std::make_pair(s,actions))*nbObservations[0]*nbObservations[1]*probHist_;
    }
    //}
    /*
    else{
...
posgFromFileGame::posgFromFileGame(const GameParameters& params)
    ...
    DecPOMDPDiscrete test("Test parse problem", "parses the test.dpomdp file ", "mabc.dpomdp");
