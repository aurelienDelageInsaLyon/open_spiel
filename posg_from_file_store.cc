...
int kNumPlayers = 2;
int kNumDistinctActions = 4;
int kNumStates = 2;
int kNumObservations = 9;
...
void posgFromFileState::DoApplyActions(const std::vector<Action>& actions) {
  ...
  if (IsSimultaneousNode()){ 
    if (timestep_>0 && false){
    std::map<int,double> tmpDistrib={{0,distribHidden_States[0]},{1,distribHidden_States[1]}};
    Action lastObsStore = lastObservations;
    Action tmpLastObservations = lastObservations+(18-lastObservations);
    evolveState(lastActions_);
    std::map<int,double> tmpDistribStore={{0,distribHidden_States[0]},{1,distribHidden_States[1]}};
    for (int i =0;i<nbStates;i++){
      distribHidden_States[i] = tmpDistrib[i];
    }
    std::vector<Action> tmpActions = {lastActions_[1],lastActions_[0]};
    lastObservations = tmpLastObservations;
    evolveState(tmpActions);
    lastObservations = lastObsStore;
        std::cout << "\n distribs : " << tmpDistribStore[0] << "," <<tmpDistribStore[1]
         << " and : " << distribHidden_States[0] << ","<<distribHidden_States[1];
    }
    reward_+= posgFromFileState::resolveRewards(actions);
    cur_player_ = kChancePlayerId;
    //std::cout << "\n players played : " << actions << " at timestep " << timestep_<<"\n";
...
posgFromFileGame::posgFromFileGame(const GameParameters& params)
    ...
    DecPOMDPDiscrete test("Test parse problem", "parses the test.dpomdp file ", "competitive_tiger.dpomdp");
